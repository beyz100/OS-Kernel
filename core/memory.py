...
class MemoryManager:
    ...
    def allocate(self, pid: int, memory_required: int) -> bool:
        ...
        if pages_needed > free_frames:
            OSLogger.log("Memory", f"Allocation FAILED: PID {pid} needs {pages_needed} pages, only {free_frames} available")
            return False
        ...
        OSLogger.log("Memory", f"Allocated {pages_needed} pages to PID {pid} ({memory_required} bytes)")
        return True
    ...
    def deallocate(self, pid: int) -> bool:
        ...
        OSLogger.log("Memory", f"Deallocated all pages for PID {pid}")
        return True
